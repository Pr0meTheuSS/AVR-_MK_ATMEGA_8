...
// Функция для преобразования показаний датчика VL53L0x в миллиметры
...
int main(void)
{
    ...
    while (1) 
    {
		...
		new_data = I2C_ReadLastByte();
	    	// При изменении показаний на очередной итерации измерений более чем на 5 миллиметров 
		if (Abs(data[1], new_data) > 5){
			data[1] = new_data;
		}
	    	// Вывод данных на 4-х разрядный 7-сегментый индикатор на драйвере TM 1637 через функцию специальной библиотеки 
